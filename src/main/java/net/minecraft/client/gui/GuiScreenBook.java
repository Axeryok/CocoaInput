...
public class GuiScreenBook extends GuiScreen implements IMEReceiver
{
    ...
    public void insertText(String aString, int position1, int length1) {
        if (this.bookGettingSigned) {
            bookTitle = new StringBuffer(bookTitle).replace(lengthBeforeMarkedText,
                                                            lengthBeforeMarkedText + length, aString).toString();
            hasMarkedText = false;
            length = 0;
        } else {
            pageSetCurrent(new StringBuffer(this.pageGetCurrent()).replace(
    ...
    public void setMarkedText(String aString, int position1, int length1,
        ...
        if (this.bookGettingSigned) {
            if (hasMarkedText == false) {
                hasMarkedText = true;
                lengthBeforeMarkedText = bookTitle.length();
            }
            bookTitle = new StringBuffer(bookTitle).replace(lengthBeforeMarkedText,
            ...
            if (hasMarkedText == false) {
                hasMarkedText = true;
                lengthBeforeMarkedText = this.pageGetCurrent().length();
            }
            pageSetCurrent(new StringBuffer(this.pageGetCurrent()).replace(
