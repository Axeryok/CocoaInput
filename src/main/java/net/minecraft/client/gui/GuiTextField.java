...
public class GuiTextField extends Gui implements IMEReceiver {
	...
	public void insertText(String aString, int position, int lengthp) {
		if (aString.length() == 0) {
			text = (new StringBuffer(text)).replace(cursorPosition, cursorPosition + length, "").toString();
			length = 0;
			return;
		}
		text = (new StringBuffer(text))
				.replace(cursorPosition, cursorPosition + length, aString.substring(0, aString.length() - 1))
				.toString();
		length = 0;
		cursorPosition += aString.length() - 1;
		selectionEnd = cursorPosition;
		try {
			Minecraft.getMinecraft().currentScreen.keyTyped(aString.charAt(aString.length() - 1),
