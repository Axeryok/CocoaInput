...
public class TinyConfig {
    ...
    protected static class EntryInfo {
        ...
        Map.Entry<TextFieldWidget,TextComponent> error;
        Object defaultValue;
        Object value;
        String tempValue;
    ...
    public static void init(String modid,Path apath, Class<?> config) {
        ...
        for (Field field : config.getFields()) {
            ...
            if (type == int.class)         textField(info, Integer::parseInt, INTEGER_ONLY, e.min(), e.max(), true);
                ...
                Function<Object,TextComponent> func = value -> new StringTextComponent((Boolean) value ? "True" : "False");
                info.widget = new AbstractMap.SimpleEntry<Button.IPressable, Function<Object, TextComponent>>(button -> {
                ...
                Function<Object,TextComponent> func = value -> new TranslationTextComponent(translationPrefix + "enum." + type.getSimpleName() + "." + info.value.toString());
                info.widget = new AbstractMap.SimpleEntry<Button.IPressable, Function<Object,TextComponent>>( button -> {
    ...
    private static void textField(EntryInfo info, Function<String,Number> f, Pattern pattern, double min, double max, boolean cast) {
        ...
        info.widget = (BiFunction<TextFieldWidget, Button, Predicate<String>>) (t, b) -> s -> {
            s = s.trim();
            ...
            if (!(isNumber && s.isEmpty()) && !s.equals("-") && !s.equals(".")) {
                ...
                info.error = inLimits? null : new AbstractMap.SimpleEntry<>(t, new StringTextComponent(value.doubleValue() < min ?
    ...
    private static class TinyConfigScreen extends Screen {
        protected TinyConfigScreen(Screen parent) {
            super(new StringTextComponent("CocoaInput config"));
            this.parent = parent;
        }
        ...
        protected void init() {
            ...
            Button done = this.addButton(new Button(this.width/2 - 100,this.height - 28,200,20,
                    new TranslationTextComponent("gui.done"), (button) -> {
                for (EntryInfo info : entries)
            ...
            for (EntryInfo info : entries) {
                if (info.widget instanceof Map.Entry) {
                    Map.Entry<Button.IPressable,Function<Object,TextComponent>> widget = (Map.Entry<Button.IPressable, Function<Object, TextComponent>>) info.widget;
                    addButton(new Button(width-85,y,info.width,20, widget.getValue().apply(info.value), widget.getKey()));
                }
                else {
                    TextFieldWidget widget = addButton(new TextFieldWidget(font, width-85, y, info.width, 20, null));
                    widget.setValue(info.tempValue);

                    Predicate<String> processor = ((BiFunction<TextFieldWidget, Button, Predicate<String>>) info.widget).apply(widget,done);
        ...
        public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            ...
            int y = 40;
            for (EntryInfo info : entries) {
				drawString(matrices, font, new StringTextComponent(info.comment), 12, y + 10, 0xFFFFFF);
				/*
